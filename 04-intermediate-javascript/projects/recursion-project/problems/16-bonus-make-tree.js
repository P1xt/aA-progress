...
const  makeTree = (categories, currentId) => {
  ...
  if (currentId === null) {
    const root = categories.find(next => next.parent === null).id;
    return { [root]: makeTree(categories, root) };
  // add each additional branch to the tree
  } else {
    let tree = {};
    categories.filter(category => category.parent === currentId)
              .map(category => category.id)
              .forEach(child => tree[child] = makeTree(categories, child));
    return tree;
  }
}
