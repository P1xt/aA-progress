...
/**
 * We check if the input is an array, if it is, we create a new array, iterate through the input array,
 * and push the result of calling deepDup on each element of the input array into the new array
 *
 * @return [1, 2, 3, 4, 5]
 */
const deepDup = arr => {
  ...
  /* base case, Checking if the input is an array. If it is not an array,
     it returns the input. */
  if (!Array.isArray(arr)) {
    return arr;
  }
  const dup = [];
  /* recursive case, Iterating through the array and pushing the result
     of calling deepDup on each element of the input
