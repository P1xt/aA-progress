...
/**
 * Return a function that takes in a number and multiplies it by the volume.
 * @returns A function that takes in a number and multiplies it by the volume.
 */
const recVolume = height => {
  let volume = height;
  let count = 1;

  /* Returning a function that takes in a number and multiplies it by the volume. */
  return getNext = next => {
    count++;
    if (count <= 3) {
      volume *= next;
